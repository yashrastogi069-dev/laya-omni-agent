...
28. **Checkpoint L14.2 Milestone Reached (Runtime Closure, Adversarial Durability & Evidence Integrity Gate)**:
    ...
    - **Attempt State Consistency & Zombie Mutation Defense (L14.2-E)**: `commit_attempt_atomic` and `fail_attempt_atomic` enforce aggregate root state validation (rejecting `UNKNOWN_COMMIT` and terminal states) and attempt state validation (must be `STARTED`). Current-attempt parity in `fail_attempt_atomic` checks `op_row["current_attempt"] != attempt.attempt_number` raising `StaleAttemptError` (`test_e5`).
    - **Plan Provenance & Deterministic Tamper Firewall (L14.2-F & G)**: Canonical SHA-256 calculation via `Plan.compute_hash()` persisted in `quest.metadata["plan_hash"]` incorporating `schema_version`, `plan_version`, `validator_version`, `validation_hash`, `validation_receipt`, and sorted `metadata`. Deterministic execution firewall in `_execute_internal` recomputes plan hash and blocks execution with `ExecutionFirewallError` if SQLite plan steps, capabilities, dependencies, phantom steps, or metadata/budgets are tampered with (0 capabilities dispatched). Generative plan types faithfully survive reconstruction.
    ...
    - **Truthful Timeout Hierarchy & Real Timeout Semantics (L14.2-J)**: ThreadPoolExecutor bounded execution per step with explicit non-blocking `pool.shutdown(wait=False, cancel_futures=True)`. CPython thread limits defined. In-flight mutation timeouts quarantined into `UNKNOWN_COMMIT` and quest paused in `PAUSED_FOR_RECONCILIATION`.
    - **Active Cancellation Protocol & Durable Intent (L14.2-K)**: Active cancellation persists durable intent `cancellation_requested = True`, stops new dispatch, drains workers, marks uncompleted steps `CANCELLED`, transitions running mutations to `UNKNOWN_COMMIT`, pauses for reconciliation, and on restart/reconciliation cleanly cancels without executing downstream steps (`test_k1`, `test_k2`).
    - **Append-Only Reconciliation History & DB CAS (L14.2-L)**: Immutable audit trail persisted in SQLite table `operation_reconciliations` with full provenance (`reconciliation_id`, `operation_id`, `prior_state`, `reconciled_state`, `evidence`, `note`, `timestamp`, `actor`). DB CAS update enforces conditional state check, rejecting conflicting reconciliations with `ConcurrentReconciliationConflictError` (`test_l2`).
    - **Policy Latency Benchmark Distribution Proof**: 20-sample statistical distribution asserting median < 5.0ms (sub-1ms SLA proof) in `tests/test_l9_policy.py`.
